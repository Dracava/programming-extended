//Map API Key
var mapApiKey = 'pk.eyJ1IjoiZHJhY2F2YSIsImEiOiJja3FhdHM4eGIwMWR2MnFzMXdyNHQzNXE0In0.wRKx2--d7pM6iGs4S_7jkQ';
mapboxgl.accessToken = mapApiKey;

//Map
var map = new mapboxgl.Map(
	{container: 'map',
  	style: 'mapbox://styles/dracava/ckqauux9r05qu18o1c4z6oxkb',
...
document.getElementById("search_button").onclick = function () {
	...
	var WeatherApiKey ='4ee62a52f85d0c038daad61163fdbd0d';
...
function onMapAPISuccess(response) {
	...
	rocket.className = 'rocketMarker';
	var landingPopup = new mapboxgl.Popup().setHTML('<h3>Landing Spot</h3>' + response.features[0].place_name);
	var marker = new mapboxgl.Marker(rocket).setLngLat([longitude, latitude]).setPopup(landingPopup).addTo(map);
...
function onWeatherAPISuccess(response) {
	...
	weatherBox.innerHTML = '<h1>' + 'Chosen location: ' + response.name + '</h1>';
